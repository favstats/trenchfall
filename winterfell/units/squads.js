...
export class Force {
  constructor(scene, state) {
    ...
    const trenchLine = WALL_Z - 22;
    const supportLine = WALL_Z - 16;
    // the first screen is a dug-in firing line; the castle is fallback terrain
    this.squads.push(new Squad(scene, '1 RIFLES', 'rifle', -70, trenchLine, 6, state));
    this.squads.push(new Squad(scene, '2 RIFLES', 'rifle', 70, trenchLine, 6, state));
    this.squads.push(new Squad(scene, '3 RIFLES', 'rifle', -26, supportLine, 6, state));
    this.squads.push(new Squad(scene, 'MG TEAM', 'mg', 24, supportLine, 3, state));
