...
import { WALL_Z, NORTH_Z, FIELD_HALF_X, GATE_W, heightAt } from '../world/field.js';
...
export class Horde {
  ...
  spawnWave(n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const z = NORTH_Z + 20 + Math.random() * 40;
  ...
  update(dt) {
    ...
    for (let i = 0; i < n; i++) {
      const a = A[i];
      // advance south toward the wall; funnel toward the gate as they close in
      let tx = a.x;
      if (a.z > WALL_Z - 30) tx = a.x * 0.985; // drift toward x=0 (the gate) near the wall
      const tz = WALL_Z + 2.5;
      ...
      o.position.set(a.x, heightAt(a.x, a.z) + bob, a.z);
      o.rotation.set(0, Math.atan2(-mvx, -mvz) + sway, sway * 0.6);
