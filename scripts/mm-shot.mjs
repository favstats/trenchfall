...
try {
  ...
  const errs=[]; p.on('pageerror',e=>errs.push(''+e)); p.on('console',m=>{if(m.type()==='error')errs.push('C:'+m.text())});
  ...
  await sleep(4500);
  await p.screenshot({ path: 'dist/shots/winterfell-embers.png', timeout: 60000, animations: 'disabled' });
