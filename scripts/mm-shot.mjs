...
try {
  ...
  await p.goto('http://localhost:5173/winterfell.html?gl=1&end=lost', { waitUntil: 'domcontentloaded' });
  await p.waitForFunction(() => window.WF?.ready === true, { timeout: 30000 }).catch(()=>{});
  await sleep(2500);
  await p.screenshot({ path: 'dist/shots/winterfell-end.png', timeout: 45000 });
