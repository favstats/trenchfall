...
const SFX={
  step(){sNoise(.07,'lowpass',rand(380,520),120,.05);},
  ...
  load(){sNoise(.04,'highpass',2400,1000,.1);sTone('square',420,520,.05,.06);setTimeout(()=>sNoise(.03,'lowpass',800,400,.08),90);},
  build(){sNoise(.1,'lowpass',700,200,.2);sTone('sine',140,90,.09,.15);
  ...
  deny(){sNoise(.07,'lowpass',420,160,.16);sTone('sine',95,70,.12,.12);},
  perk(){
    const t0=AU.ctx?AU.ctx.currentTime:0;
    [[98,1.2,.1],[147,1.2,.08],[196,1.4,.09],[294,1.6,.05]].forEach(([f,d,v],i)=>sTone('sawtooth',f*.99,f,d,v,t0+i*.06));},
  beep(){sTone('sine',1240,1236,.12,.08);sTone('sine',2480,2470,.07,.025);},
...
const gunModels=[];
{
  ...
  for(let i=0;i<6;i++){
    ...
    if(i!==5){ // shared gun furniture: sights, bolt, trigger guard, grip
...
function continueCampaign(){
  ...
  G.scrap=s.scrap;G.items=s.items;player.owned=s.owned;player.reserve=s.reserve;player.maxhp=s.maxhp;
...
function renderInv(){
  ...
  const showDetail=()=>{
    ...
    if(sel.k==='w'){
      ...
      dd.textContent=w.ds+'  Magazine '+player.mags[sel.i]+'/'+w.magSize+' · reserve '+player.reserve+'.';
      const stats=[['DAMAGE',w.flame?.8:clamp((w.dmg*w.pellets)/85,0,1)],
        ['RATE',clamp(.045/w.rate,0,1)],['RANGE',clamp((w.range||90)/200,0,1)],
  ...
  WEAPONS.forEach((wp,i)=>{
    ...
    item(wp.name,'mag '+player.mags[i]+'/'+wp.magSize,
...
function updateHUD(dt){
  ...
  $('mag').textContent=player.tool?'-':(player.reloadT>0?'--':magN);
  $('mag').style.color=player.tool||player.reloadT>0?'':(magN<=magMax*.25?'#e8742c':'');
